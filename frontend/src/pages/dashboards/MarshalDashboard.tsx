...
import { ShieldCheck, AlertOctagon, ArrowLeft, Camera, Upload, X, Search, DoorOpen, Users, FileText, ChevronRight } from 'lucide-react';
...
export const MarshalDashboard = () => {
  ...
  const handleComplaintSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to log complaint';
      alert(message);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {incidents.length === 0 ? (
            <div className="col-span-full text-center py-8">
              <p className="text-gray-500 dark:text-gray-400">No incidents logged yet.</p>
            </div>
          ) : (
            incidents.map((incident) => (
              <div key={incident.id} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg border border-gray-200 dark:border-gray-600 flex gap-4">
                {incident.photoUrl && (
                  <img src={incident.photoUrl} alt="Incident" className="w-24 h-24 object-cover rounded-lg border border-gray-100 dark:border-gray-600" />
                )}
                <div className="flex-1">
